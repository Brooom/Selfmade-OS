...
void format_string(const char * input, char *output, int max_output_size, va_list va_list)
{
    int idx_input = 0;
    int idx_output = 0;
    while(input[idx_input] != '\0' && idx_output<max_output_size) {
        if(input[idx_input] == '%'){
            idx_input++;
            ...
            if(input[idx_input]=='s'){
                ...
                while(*arg && idx_output < max_output_size){
            ...
            //replace single character
                ...
                if(arg && idx_output < max_output_size){
            ...
            //replace unsigned decimal integer
                ...
                while(string_param[idx_param] && idx_output < max_output_size && idx_param<10){
            ...
            //replace pointer in hexadecimal
                ...
                int_to_hex_string(&param, sizeof(param), string_param);
                while(string_param[idx_param] && idx_output < max_output_size && idx_param<18){
                    output[idx_output] = string_param[idx_param];
                    idx_output++;
                    idx_param++;
                }
